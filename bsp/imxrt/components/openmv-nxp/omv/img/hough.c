...
static inline uint8_t is_edge(uint8_t *im, int x, int y)
{
    if (x < 1 || x > 320 - 2)
        return 0;
    if (y < 1 || y > 240 - 2)
...
static inline void plusuint16(uint16_t *arr, uint32_t idx)
{
    arr[idx]++;
}
static inline uint16_t readuint16(uint16_t *arr, uint32_t idx)
{
    return arr[idx];
}
void imlib_find_lines(list_t *out, image_t *ptr, rectangle_t *roi, unsigned int x_stride, unsigned int y_stride,
                      uint32_t threshold, unsigned int theta_margin, unsigned int rho_margin) // x_stride = white_thresh , y_stride = black_thresh , theta_margin = h_low , rho_margin = h_high
{
    uint8_t *grayscale_image = fb_alloc(roi->w * roi->h, FB_ALLOC_NO_HINT);
    uint16_t *score = fb_alloc(60 * 200 * sizeof(uint16_t), FB_ALLOC_NO_HINT); // 只检测-15~+14和75~104两个方向上的直线,rmax = 200;
    uint8_t h, s, v;
    uint8_t *img = grayscale_image;
    for (int y = roi->y, yy = roi->y + roi->h; y < yy; y++)
    {
        ...
        for (int x = roi->x, xx = roi->x + roi->w; x < xx; x++)
        {
            ...
            if (s < x_stride || v < y_stride)
            {
                *(grayscale_image++) = 255;
            }
            else if (h >= theta_margin && h <= rho_margin)
            {
                *(grayscale_image++) = 0;
    ...
    memset(score, 0, sizeof(uint16_t) * 60 * 200);
    float r;
    for (int y = roi->y, yy = roi->y + roi->h; y < yy; y++)
    {
        for (int x = roi->x, xx = roi->x + roi->w; x < xx; x++)
        {
            if (is_edge(img, x, y))
            {
                ...
                for (int k = 0; k < 30; k++)
                {
                    int16_t theta_idx = k - 15;
                    if (theta_idx < 0)
                        theta_idx += 360;
                    r = sin_table[theta_idx] * y + cos_table[theta_idx] * x;
                    if (r > 0)
                    {
                        // mp_printf(&mp_plat_print, "r");
                        r = fast_roundf(r / 2);
                        plusuint16(score, k * 200 + (int)r);
                    }
                }
                for (int k = 30; k < 60; k++)
                {
                    int16_t theta_idx = k + 45;
                    r = sin_table[theta_idx] * y + cos_table[theta_idx] * x;
                    if (r > 0)
                    {
                        // mp_printf(&mp_plat_print, "r");
    ...
    uint16_t maxcnt = 0;
    uint8_t theta_max = 0, r_max = 0;
    for (uint8_t i = 0; i < 30; i++)
    {
        for (uint8_t j = 0; j < 200; j++)
        {
            if (maxcnt < readuint16(score, i * 200 + j))
            {
                maxcnt = readuint16(score, i * 200 + j);
                theta_max = i;
                r_max = j;
            }
        }
    ...
    // mp_printf(&mp_plat_print, "max%d\n", maxcnt);
    if (maxcnt > threshold)
    {
        find_lines_list_lnk_data_t lnk_line;
        memset(&lnk_line, 0, sizeof(find_lines_list_lnk_data_t));

        lnk_line.magnitude = maxcnt;
        lnk_line.theta = (theta_max + 345) % 360;
        lnk_line.rho = r_max * 2;
                        ...
                        unsigned int r_min, unsigned int r_max, unsigned int r_step)
{
    ...
    {                                         // ignore r = 0/1
        ...
        for (int y = 1, yy = b_size - 1; y < yy; y++)
        {
            ...
            {
                val = row_ptr[x];
                if ((val >= threshold) && (val >= row_ptr[x - a_size - 1]) && (val >= row_ptr[x - a_size]) && (val >= row_ptr[x - a_size + 1]) && (val >= row_ptr[x - 1]) && (val >= row_ptr[x + 1]) && (val >= row_ptr[x + a_size - 1]) && (val >= row_ptr[x + a_size]) && (val >= row_ptr[x + a_size + 1]))
                {
